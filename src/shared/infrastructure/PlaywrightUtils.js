class SolutionBoxProduct {
  ...
  async setBrand() {
    const brand = await (
      await this.listItem.$('[itemprop="brand"]')
    ).textContent();
    this.brand = brand.trim();
  }

  async setStock() {
    const stock = await this.listItem.$("text=Stock");
  ...
  async setProductPageUrl() {
    const productPageUrl = await this.listItem.$eval(
      "#ver-producto",
      (el) => el.href
    );
...
async function solutionBoxGetProducts({
  page,
  categoryUrl,
  category,
  updateCallback,
  ...
  const totalPages = pagePaginationLIs.length
    ? pagePaginationLIs.length - 2
    : 1;
  ...
  for (let pageNumber = 1; pageNumber < totalPages + 1; pageNumber++) {
    ...
    for (const product of productList) {
      const solutionBoxProduct = new SolutionBoxProduct(
        product,
        category,
        categoryUrl
      );
...
export { solutionBoxCategoriesListToJson, solutionBoxGetProducts };
