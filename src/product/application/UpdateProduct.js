...
export function makeUpdateProduct({
  ...
  return async (_id, productData, files) => {
    ...
    const imagesURLsToDelete = productUpdated.images.filter(
      (image) => !imagesURLsThatPersist.includes(image)
    );

    await storageRepository.deleteImages(imagesURLsToDelete);

    const imagesURLs = await storageRepository.uploadImages(
      files,
      `products/${_id}`
    );
